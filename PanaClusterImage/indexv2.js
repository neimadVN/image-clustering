...
async function main() {
  ...
  // Extend page limit
  const extendedUrl = `${url}&view_list=12000`;

  try {
    const $ = await fetchHTML(extendedUrl);

    // Extract the search condition text
    const pElement = $('.search-cond-text');
    const spanTexts = pElement.find('span').map((index, element) => $(element).text()).get();
    const titleOfCategory = spanTexts.join(' ').replace(/[/\\?%*:|"<>]/g, '-');
